...
export class NewAddAssignmentComponent implements OnInit, OnDestroy {
  ...
  onSubmit() {
    this.loading = true;
    const thing = new Assignment();
    thing.nom = this.thingForm.get('nom').value;
    thing.dateRendu = this.thingForm.get('dateRendu').value;
    thing.auteur = this.thingForm.get('auteur').value;
    thing.imageUrl = this.thingForm.get('imageUrl').value;
    thing.remarques = this.thingForm.get('remarques').value;
    thing._id = new Date().getTime().toString();
    thing.userId = this.userId;
    this.stuffService.createNewassignment(thing).then(
      () => {
